...
In this chapter, we will look into how to put values into something called a variable. But, we need to learn a bit more about programs before we add that feature. 
\section{More output}
\index{output}
\index{statement!output}
First, we will try making a more complex program with more output statements and comment lines.
As mentioned in the last chapter, you can put as many statements as
...
\end{lstlisting}
This shows two output lines and many comments. As you can see, it is legal to put comments at the
...
Often it is useful to display the output from multiple output statements all on one line.  You can do this by leaving out
the first {\tt endl}:
